...
import com.echoai.domain.VectorAccumulator
...
class MainActivity : AppCompatActivity() {
    ...
    // Rotational-aperture localizer using vector summation. Each window contributes a
    // 2D vote `bot_ild × (cos yaw, sin yaw)`; the accumulated vector points at the
    // estimated world-frame source direction with magnitude as confidence. Replaces the
    // earlier cos-Bayesian BeliefDistribution (preserved in git stash@{0}).
    private val accumulator = VectorAccumulator(
        decayPerWindow = 0.10f,
        magnitudeFloor = 0.30f,
        flipSign = false,
    )

    /** Empirical max accumulator magnitude we expect, for normalizing to [0, 1] for the
     *  radar arrow length. Calibrate from `pointer_magnitude` in the diagnostics CSV. */
    private val pointerMagnitudeMax: Float = 2.5f
    ...
    private fun onResetBelief() {
        ...
        accumulator.reset()
        // Push a null pointer to the radar immediately so the arrow clears without
        // waiting for the next window emit.
        binding.radar.setPointer(null, 0f, orientationProvider.yawDegrees() ?: 0f)
    }
        ...
        pipelineJob = lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                captureManager.windows.collectLatest { window ->
                    ...
                    binding.radar.setPointer(frame.pointerDirection, frame.pointerMagnitudeNorm, frame.phoneYaw)
                }
            }
        }
    }
    ...
    private fun stopLive() {
        ...
        binding.radar.setPointer(null, 0f, 0f)
    ...
    private data class Frame(
        ...
        val pointerDirection: Float?,
        val pointerMagnitudeNorm: Float,
        val phoneYaw: Float,
    )
    ...
    private suspend fun processWindow(window: AudioWindow): Frame = coroutineScope {
        ...
        if (yaw != null) {
            accumulator.update(multi.full.bottomIld, yaw)
        }

        val pointerDirection = accumulator.directionDegrees()
        val pointerMagnitudeRaw = accumulator.magnitude()
        val pointerMagnitudeNorm = (pointerMagnitudeRaw / pointerMagnitudeMax).coerceIn(0f, 1f)
        ...
        diagnosticsLogger?.log(
            ...
            azimuthLagDeg = azLag,
            phoneYawDeg = yaw,
            pointerDirDeg = pointerDirection,
            pointerMagnitude = pointerMagnitudeRaw,
        )

        Frame(
            text = renderLive(window, classification.topK, multi.full, multi.sub, events, yaw, pointerDirection, pointerMagnitudeRaw),
            events = events,
            pointerDirection = pointerDirection,
            pointerMagnitudeNorm = pointerMagnitudeNorm,
            phoneYaw = yaw ?: 0f,
        )
    }
    ...
    private fun renderLive(
        ...
        appendLine("Arrow points at estimated world-frame source direction (device-frame transformed).")
        appendLine()

        appendLine("IMU / pointer:")
        appendLine("  phone yaw  ${yawDegrees?.let { "%6.1f°".format(it) } ?: "    ?  "}  (world heading of TOP edge)")
        appendLine(
            "  pointer    ${pointerDirection?.let { "%6.1f°".format(it) } ?: "  none "}  magnitude=${"%.3f".format(pointerMagnitude)}  (floor=0.30, expected_max≈2.5)"
        )
        appendLine()
